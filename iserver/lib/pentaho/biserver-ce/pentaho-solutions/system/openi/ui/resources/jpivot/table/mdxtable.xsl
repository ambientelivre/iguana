...
<xsl:template match="mdxtable">
  ...
  <table border="0" cellspacing="1" cellpadding="5" id="{$renderId}">
    ...
    <!-- 
	    <xsl:choose>
			<xsl:when test="@sortable = 'true'">
				...
				<ul id="sortMenu-{@id}" class="contextMenu">
				    <li>
				        <a href="" onclick="alert({@id}_naturalSort);">Natural Order</a>
				    </li>
				    <li>
				        <a href="" onclick="alert({@id}_sortAsc);">Sort Low -> High</a>
				    </li>
				    <li>
				        <a href="" onclick="alert({@id}_sortDesc);">Sort High -> Low</a>
				    </li>
				</ul>
				<script type="text/javascript" charset="utf-8">
					 <![CDATA[
						jQuery(document).ready( function() {
						    jQuery("#sortable-{@id}").contextMenu({
						        menu: "sortMenu-{@id}"
						    },
...
<xsl:template match="drill-expand | drill-collapse">
  <input type="image" title="{@title}" id="{@id}" name="{@id}" src="jpivot/table/{@img}" border="0" onclick="OlapActions.drillExpandCollapse(this, pivotID); return false;" />
  <xsl:text> </xsl:text>
</xsl:template>

<xsl:template match="drill-other">
  <img src="jpivot/table/{@img}" border="0" width="9" height="9"/>
</xsl:template>

<!-- navigation: sort -->
<xsl:template match="sort">
  <input name="{@id}" title="{@title}" type="image" src="jpivot/table/{@mode}.gif" border="0" width="9" height="9" onclick="OlapActions.sort(this, pivotID); return false;" />
</xsl:template>

<xsl:template match="drill-through">
  <input name="{@id}" title="{@title}" type="image" src="jpivot/table/drill-through.gif" border="0" width="9" height="9" onclick="OpenIAnalysis.showDrillThroughOptionDialog(this, pivotID); return false;" />
</xsl:template>
...
<xsl:template match="property[@name='arrow']">
  <span style="margin-left: 0.5ex">
    <img border="0" src="jpivot/table/arrow-{@value}.gif" width="10" height="10"/>
  </span>
</xsl:template>
...
<xsl:template match="property[@name='cyberfilter']">
  <span style="margin-left: 0.5ex">
    <img align="middle" src="jpivot/table/filter-{@value}.gif" width="53" height="14"/>
  </span>
</xsl:template>
